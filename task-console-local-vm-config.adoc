...
summary: 您可以在部署 Console 本地部署后对现有 VCenter 或 ESXi 主机进行更改。例如，您可以增加托管 Console 本地部署的 VM 实例的 CPU 或 RAM。 
---
...
+
.. 选择 *Hard Disk 2* 以增加用于 /opt 的硬盘空间。
.. 选择 *Hard Disk 3* 以增加用于 /var 的硬盘空间。
...
. 选择 *Launch Web Console*。
. 使用创建 VM 实例时指定的用户名和密码登录到 VM 实例。用户名是 `maint`，密码是您在创建 VM 实例时指定的密码。
. 输入 `1 to view the `System Configuration`菜单。
. 输入 `2`并按照屏幕提示操作。控制台扫描新设置并增加分区大小。
